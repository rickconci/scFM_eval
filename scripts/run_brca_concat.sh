#!/bin/bash
# =============================================================================
# Run concatenated scSimilarity + scConcept embeddings for Bassez (brca_full) dataset
# =============================================================================
# 
# Workflow:
#   1. Prepare dataset (combine cohorts)
#   2. Extract embeddings ONCE for ALL cells (expensive, GPU)
#   3. Run classification tasks using pre-computed embeddings (fast, CPU only)
#
# Usage:
#   cd /lotterlab/users/riccardo/ML_BIO/SCFM_meta
#   CUDA_VISIBLE_DEVICES=0 bash /lotterlab/users/riccardo/ML_BIO/scFM_repos/scFM_eval/run/run_brca_concat.sh
# =============================================================================
...
# =============================================================================
# Step 2: Run classification tasks (using pre-computed embeddings)
# =============================================================================
echo "=========================================="
echo "Step 3: Classification tasks (fast, using pre-computed embeddings)"
echo "=========================================="
echo ""
...
# echo "----------------------------------------"
# (cd "${UV_PROJECT_DIR}" && uv run python "${SCFM_EVAL_DIR}/run/run_exp.py" brca_full/subtype/concat_sim_concept.yaml)
# echo ""

# #--------------------------------Chemo (treatment_naive vs neoadjuvant_chemo)---------------------------------------------
# echo "----------------------------------------"
# echo "Task: Chemo (treatment_naive vs neoadjuvant_chemo)"
# echo "----------------------------------------"
# (cd "${UV_PROJECT_DIR}" && uv run python "${SCFM_EVAL_DIR}/run/run_exp.py" brca_full/chemo/concat_sim_concept.yaml)
# echo ""
...
echo "----------------------------------------"
(cd "${UV_PROJECT_DIR}" && uv run python "${SCFM_EVAL_DIR}/run/run_exp.py" brca_full/cell_type/concat_sim_concept.yaml)
echo ""
